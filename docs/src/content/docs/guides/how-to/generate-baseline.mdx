...
If you want to create your own baseline or modify an existing one, run `generate-baseline.py` from the scripts folder. This generates a `{baseline}.yaml` file containing all the rules that match the provided tag (baseline). You need this `{baseline}.yaml` file to run the `generate-guidance.py` script.
