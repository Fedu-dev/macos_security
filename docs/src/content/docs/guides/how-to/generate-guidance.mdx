...
To generate the guidance files (AsciiDoc, HTML, and PDF), run the `generate_guidance.py` script and point it to one of the built-in `baseline.yaml` files. You can also use a custom `baseline.yaml` file that you created with the `generate_baseline.py` script.
