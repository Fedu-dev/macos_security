...
Add the `-s` flag to the `generate-guidance.py` script to generate a compliance script, written for Z Shell (zsh), that you can use to check or fix a system.
...
When you generate the compliance script, the process creates an `org.{baseline}.audit.plist` file. This file represents the structure for implementing exemptions. See the section on [Exemptions](https://github.com/usnistgov/macos_security/wiki/exemptions) for more information.
...
After running the script, the results write to `/Library/Preferences/org.{baseline}.audit.plist` and `/Library/Logs/{baseline}_baseline.log`.
