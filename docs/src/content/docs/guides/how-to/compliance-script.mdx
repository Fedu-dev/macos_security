...
Add the `-s` flag to the `generate-guidance.py` script to generate a compliance script. The script uses Z Shell (zsh) and you can run it to check or fix a system.
...
**💡 TIP**\
To add a custom reference in front of the rule in the `{baseline}_baseline.log`, use the `-r` flag when you run the `generate_guidance.py` script. The custom reference value is **case sensitive**.

## Compliance Script Command Line Argument Reference

**📌 NOTE**\
The compliance script requires Z Shell (zsh). Running it with `sh` or `bash` produces errors.
