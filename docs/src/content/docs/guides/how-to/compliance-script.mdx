...
Add the `-s` flag to the `generate-guidance.py` script to generate a compliance script. The script is written for Z Shell (zsh) and you can use it to check or fix a system.
...
When you generate the compliance script, the process creates an `org.{baseline}.audit.plist` file. You use this file to implement exemptions. See the section on [Exemptions](https://github.com/usnistgov/macos_security/wiki/exemptions) for more information.

You can run the `{baseline}_compliance.sh` script interactively or with the --check/--fix/--cfc flags. To run the script interactively, use the following command:
...
**📌 NOTE**\
The `--cfc` flag triggers a check/fix/check when you run the compliance script.

After you run the script, it writes the results to `/Library/Preferences/org.{baseline}.audit.plist` and `/Library/Logs/{baseline}_baseline.log`.

**📌 NOTE**\
You must run a successful compliance scan before the script can remediate non-compliant settings.
...
**💡 TIP**\
To add a custom reference in front of the rule in the `{baseline}_baseline.log`, use the `-r` flag when running the `generate_guidance.py` script. The custom reference is **(case sensitive)**.

## Compliance Script Command Line Argument Reference

**📌 NOTE**\
The compliance script expects Z Shell (zsh). Running it with `sh` or `bash` results in errors.
